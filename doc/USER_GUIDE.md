...
1. 在 "Key Notes" 树中，右击术语分组可以将它设为当前激活分组。分组行也支持重命名和删除，和书签分组保持一致。
2. Key Notes 树保留在左侧，Note Editor 保留在面板区域中。单击树项会在编辑器中加载该笔记，便于预览或继续编辑。
...
6. 在 "Key Notes" 视图中，右击笔记可以将其从当前分组移除、加入其他分组，或直接删除笔记正文/内容以及它在所有分组中的全部关联。
