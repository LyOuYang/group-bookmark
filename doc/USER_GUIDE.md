...
3. 此时侧边栏底部（或面板区）会自动呈现独立的 **Key Note Editor** 的 Webview 视图。
4. 在此使用 Markdown 记录该术语的意图、使用场景和外部链接。

**浏览和编辑**：
1. **自动选区追踪**：当你在代码中用光标再次选中这个记录过的词汇时，系统会自动从知识库寻回这篇笔记，在 Note Editor 中免打扰地静默渲染出相关上下文。
2. **全局搜索联动**：使用快捷键唤出 `Group Bookmarks: Search Key Notes...`，输入关键词搜索全局笔记库，点击后即在专属编辑器中打开。
3. **右键管理体系**：在 `Key Notes` 树视图中，你可以像管理书签一样对其使用**拖拽排序**、右键呼出 `Sort By...` 切换排序模式 (A-Z、自定义等) 或执行跨组移动等快捷操作。
