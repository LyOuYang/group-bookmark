...
1. 在 "Term Notes" 树中，右击术语分组可以将它设为当前激活分组。分组行也支持重命名和删除，和书签分组保持一致。
2. Term Notes 树保留在左侧，Note Editor 保留在面板区域中。单击树项会在编辑器中加载该笔记，便于预览或继续编辑。
3. 笔记行提供常用快捷操作；删除等破坏性操作会在执行前要求确认。
4. 编辑器右键菜单中，书签和术语笔记使用统一的标记样式。
5. 在编辑器中选中一个术语，使用 `Add Note for Selection`，会优先使用当前激活的术语分组；如果当前没有激活分组，系统会提示你选择或创建一个分组。
6. 在 "Term Notes" 视图中，右击笔记可以将其从当前分组移除、加入其他分组，或直接删除笔记正文/内容以及它在所有分组中的全部关联。
7. 选中已匹配的术语时，系统会在树中定位该笔记，并在 Note Editor 中以查看模式加载，便于快速确认内容。
