...
1. 在 "Term Notes" 视图中，右击一个术语分组可以将它设为当前激活分组。
2. 在编辑器中选中一个术语，使用 `Add Note for Selection`，会优先使用当前激活的术语分组；如果当前没有激活分组，系统会提示你选择或创建一个分组。
3. 在 "Term Notes" 视图中，右击笔记可以将其从当前分组移除、加入其他分组，或直接删除笔记正文/内容以及它在所有分组中的全部关联。
4. 选中已匹配的术语时，编辑器会显示笔记预览和关联分组，便于快速确认内容。
