...
- 新 view id：`groupTermNotesView`
- 新 view name：`Term Notes`
...
### 7.3 TermNoteGroupRelation

```ts
interface TermNoteGroupRelation {
  id: string;           // `${termNoteId}_${groupId}`
  termNoteId: string;
  groupId: string;
  order: number;
  createdAt: number;
...
```ts
interface TermNotesData {
  version: string;
  notes: TermNote[];
}

interface TermNoteGroupsData {
  version: string;
  groups: TermNoteGroup[];
}

interface TermNoteRelationsData {
  version: string;
...
- `groupBookmarks.unsetActiveTermNoteGroup`
...
- `groupBookmarks.refreshTermNotes`
...
建议为 `Term Notes` 单独实现一套 Tree Provider，而不是在现有 `BookmarkTreeProvider` 上继续扩展。两者虽然数据形态相似，但命令语义与交互边界已经明显不同，独立实现会更清晰，也更易维护。
...
```text
groupbookmarks-term-note:/<noteId>.md
```
...
当前扩展在 `src/extension.ts` 中初始化的是以 bookmark 为中心的服务。新增词条笔记后，需要并行加入一套 term-note 相关服务。

大概率会新增：

- `src/models/types.ts` 中新增类型，或拆出独立的 term-note 类型文件
- `src/data/storageService.ts` 增加词条笔记读写能力
- `src/data/dataManager.ts` 扩展 term-note 缓存和事件，或新增独立的 term-note data manager
- `TermNoteManager`
...
- 是扩展现有 `DataManager`，还是单独做一个 term-note data manager
