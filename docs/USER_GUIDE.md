...
**移动书签**：
- 拖拽书签可改变顺序或移动到其他展开的组中
- 右击书签 → 选择 "Move to Group" 可一键将其跨组转移

**书签排序**：
- 可通过直接鼠标拖放。
- 或者右击书签选择 "Move Up", "Move Down", "Move to Top" 等精确操作。

### 5. 术语笔记

### 5. 术语笔记 (Key Notes)

当你面对陌生的业务词汇、表名或者框架概念时，可以把它们沉淀为“术语笔记”。这相当于你在此项目专属的代码维基百科。

**添加新笔记**：
1. 在代码中用鼠标选定一个陌生的概念（例如 `validateUserCredentials`）。
2. 按快捷键或在右键菜单中选择 `🔖 Add Key Note`（如果没有激活的分组，会引导你新建一个组）。
3. 如果脱离代码，您还可以点击已有 Key Note 分组面上的 `+` ("Add Note") 图标，随想随打直接创建词条。
4. 此时侧边栏底部（或下方独立的面板区）会自动呈现专属的 **Key Note Editor** 的 Webview 视图。
5. 在此使用 Markdown 记录该术语意图的时候：**支持将外部应用的表格(包含 Excel 或以逗号段分隔的 CSV / TSV 数据) 直接粘贴，系统将会自动帮您转换成标准的 Markdown 表格。**

**浏览和编辑**：
1. **自动选区追踪**：当你在代码中用光标再次选中这个记录过的词汇时，系统会自动从知识库寻回这篇笔记，在 Note Editor 中免打扰地静默渲染出相关上下文。
2. **全局搜索联动**：使用 `Group Bookmarks: Search Key Notes...` 命令或点击标题栏放大镜按钮，输入关键词全局搜索笔记，选定后即刻定位渲染。
3. **右键管理体系**：在 `Key Notes` 树视图中，你可以像管理书签一样对其使用**拖拽排序**，并拥有更高阶的掌控力：
   - 右键任意 Key Note 分组呼出 `Sort By...` 在自定义拖拽、名称 A-Z、名称 Z-A 中实时切换。
   - 对独立的词条项，通过 `Alt+Up/Down` 快捷键或右键迅速完成置顶、置底以及上/下级移位。
